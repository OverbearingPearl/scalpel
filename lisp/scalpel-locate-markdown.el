...
(defconst scalpel-locate-markdown--fence-regexp
"^[ \t]*\\(`\\{3,\\}\\|~\\{3,\\}\\)"
"Regexp matching a markdown code fence delimiter line.
The line may carry leading whitespace, and group 1 captures the
run of three or more backticks or tildes that forms the fence.

Fences are the only markdown structure with a real begin/end, so
